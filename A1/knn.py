...
def compute_distances_two_loops(x_train, x_test):
  ...
  for test in range(num_test):
    for train in range(num_train):
      dists[train,test] = torch.sqrt(torch.sum(torch.pow(x_train[train]-x_test[test],2)))
...
def compute_distances_one_loop(x_train, x_test):
  ...
  # Replace "pass" statement with your code
  for test in range(num_test):
    dists[:,test] = (((x_train.view(num_test,-1) - x_test[test].view(1,-1))**2).sum(dim=1))**(1/2)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_train_2 = x_train.view(num_train,-1)**2
  x_test_2 = x_test.view(num_test,-1)**2
  x_train_test_2 = x_train_2.sum(dim=1).view(num_train,-1) + x_test_2.sum(dim=1) #m x 1 + n x 1 => pow of 2 for the index, [train,test]
  row_wise_sum = torch.mm(x_train.view(num_train,-1), x_test.view(num_test,-1).transpose(0,1)) #[train, test] = sum of elements in each train, test row
  dists = (x_train_test_2 - 2*row_wise_sum)**(1/2)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
    y_pred[i] = y_train[dists.topk(k=k, dim=0, largest=False).indices[:,i]].mode().values
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  x_train_folds=list(x_train.chunk(num_folds, 0))
  y_train_folds=list(y_train.chunk(num_folds, 0))
  ...
  for k in k_choices:
    ...
    for fold in range(num_folds):
      ...
      x_train_nocat = x_train_folds[:fold] +x_train_folds[fold+1:]
      y_train_nocat = y_train_folds[:fold] +y_train_folds[fold+1:]
      
      x_train = torch.cat(x_train_nocat, dim=0)
      y_train = torch.cat(y_train_nocat, dim=0)
      k_class = KnnClassifier(x_train, y_train)
      k_accuracy.append(k_class.check_accuracy(x_val, y_val, k=k, quiet = True))
...
def knn_get_best_k(k_to_accuracies):
  ...
  for i in k_to_accuracies:
    accuracy = sum(k_to_accuracies[i])/len(k_to_accuracies[i])
    if max < accuracy:
      max = accuracy
      best_k = i
  ##############################################################################
